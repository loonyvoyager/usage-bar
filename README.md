...
2. It's **signed and notarized**, so it just opens — no right-click, no Gatekeeper
   warning.
3. It's menu-bar-only by default (no window, and no Dock icon unless you turn one
   on in settings). Click the gauge near your clock and **sign in to claude.ai
   once**; after that it stays signed in.
